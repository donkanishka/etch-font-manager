...
- **A CSS variable per family** — by default a family is published as `--efm-family-{slug}`, so `"Noto Sans Sinhala"` is usable as `var(--efm-family-noto-sans-sinhala)` in an Etch style record, an ACSS override or any stylesheet. The value carries the family's fallback stack. Type a short name such as `sans` in the family editor to replace the generated name with `--sans`, then copy `var(--sans)` from the same field. Heading and body assignments automatically point at whichever name the family publishes.
